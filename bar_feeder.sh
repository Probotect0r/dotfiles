...
volume() {
  amixer get -D pulse Master | sed -n 'N;s/^.*\[\([0-9]\+%\).*$/\1/p'
...
while :; do
  buf=""
  buf="${buf} [$(groups)]"
  buf="${buf}%{B#242120} %{r}$(clock)"
  buf="${buf} %{O20}NET: $(network)"
  buf="${buf} %{O20}CPU: $(cpuload)%"
  buf="${buf} %{O20}RAM: $(memused)%"
  buf="${buf} %{O20} $(volume) %{O100}"
